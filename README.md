...
- 同意后显示当前步骤、进度、已下载/总大小、实时速度、连接次数和等待原因。下载前检查 HTTPS 重定向、远端大小与断点支持；15 秒没有新数据会显示停滞时长。失败自动重试，运行环境直接写入稳定 `.part` 文件，取消或退出后可跨启动断点续传，并在下次启动显示上次失败原因后重新询问。
- Mac 首次启动先询问使用 MLX 还是 Ollama；推荐针对 Apple 芯片优化的 MLX，也可随时在设置中切换。
- MLX 会按需建立独立的 MLX-VLM 环境并下载 4-bit 视觉模型，不修改系统 Python；已经安装的 uv、运行环境和模型会直接复用。模型完成后保存固定版本文件清单，后续逐个核对全部模型文件及长度。Ollama 仍作为兼容回退。
